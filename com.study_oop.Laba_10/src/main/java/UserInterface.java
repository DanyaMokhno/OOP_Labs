...
public class UserInterface {
    ...
    private void addRow() {
        ...
        log.debug("Общее количество строк после добавления: " + tableModel.getRowCount());
    }
    ...
    private void delRow() {
        if (table.getSelectedRow() != -1) {
            log.info("Удаление" + getSelectedRows().length + " строк: " + java.util.Arrays.toString(getSelectedRows()));
    ...
    private void search() {
        ...
        try {
            ...
            return; // Возвращаемся, если текст пустой или совпадает с плейсхолдером
            // Если текст пустой или placeholder, выводим предупреждение
        }

        boolean found = false; // Инициализация переменной после проверки пустого ввода
        ...
        for (int i = 0; i < table.getRowCount(); i++) {
            ...
            if (rowContainsSearchText) {
                ...
                table.scrollRectToVisible(table.getCellRect(i, 0, true)); // Прокручиваем к строке
